// Seeds birds.db with the three pedigrees already produced by hand
// (ZA 2805, ZA 2818, ZA 2911 — see build brief §6 step 1) plus a fourth
// small fixture ("The 404") used only to exercise the 2x2 line-breeding
// case, and two standalone reference birds that exist purely to pin down
// two of the "known issues to carry over" (§7):
//
...
// Running the real extraction pipeline against actual scans will populate
...
import type { Bird, Sex } from '../../shared/types.js';
...
const za2805 = mkBird({
  id: 'za2805',
  ring: 'ZA 2805 BORD 2026',
  sireId: za2805Sire.id,
  damId: za2805Dam.id,
});
...
// Same ring text as ZA 2911's dam-side ancestor below -> same ringNormalised
// -> Phase 5 shared-ancestor match, even though the two rows have different
// ids (each upload produced its own copy of her box).
...
// Demonstrates §7 / §1 "empty box stays empty": her dam was not recorded on
// the source sheet, so damId is left unset rather than guessed.
...
const za2818DamDam = mkBird({
  ...
  // damId intentionally omitted — unrecorded on source, not fabricated.
...
const za2818 = mkBird({
  id: 'za2818',
  ring: 'ZA 2818',
  sireId: za2818Sire.id,
  damId: za2818Dam.id,
});

// ---------------------------------------------------------------------------
// ZA 2911 — DV 07274-08-817 "Nelly" sits 4x5 in the sire's own pedigree
// (four sire-line generations one way, five dam-line generations the
// other, converging on the same ring). Also carries the BE 14-1034371
// shared ancestor.
// ---------------------------------------------------------------------------

// Pure sire-line, 4 generations deep, ending in Nelly (occurrence A).
...
// Pure dam-line, 5 generations deep, ending in Nelly (occurrence B) —
// starts from the shared ancestor Mother Best Kittel.
...
const za2911 = mkBird({
  id: 'za2911',
  ring: 'ZA 2911',
  sireId: za2911Sire.id,
  damId: za2911Dam.id,
});

// ---------------------------------------------------------------------------
// "The 404" — bonus fixture, exists only to demonstrate the 2x2 case:
// BEL 17-1019414 "Junior" sits 2x2 (two grandparent slots).
// ---------------------------------------------------------------------------
...
const the404 = mkBird({
  ...
  ring: 'ZA 22 OUDL 404',
  name: 'The 404',
  sireId: the404Sire.id,
  damId: the404Dam.id,
});

// ---------------------------------------------------------------------------
// Standalone reference bird — §7: a missing race result must render as a
// visible red placeholder, never be quietly omitted. Not wired into any
// pedigree tree; exists only so the render/verify UI has a real example of
// this state to show.
// ---------------------------------------------------------------------------
...
function seedChildPedigree(childId: string, sireRootId: string, damRootId: string, sireFile: string, damFile: string) {
  const sireUploadId = randomUUID();
  const damUploadId = randomUUID();
  saveUpload({ id: sireUploadId, originalFilename: sireFile, storedPath: `data/uploads/${sireFile}`, rootBirdId: sireRootId, verified: true });
  saveUpload({ id: damUploadId, originalFilename: damFile, storedPath: `data/uploads/${damFile}`, rootBirdId: damRootId, verified: true });
  saveChildPedigree({
    id: randomUUID(),
    childBirdId: childId,
    sireUploadId,
...
seedChildPedigree(za2805.id, za2805Sire.id, za2805Dam.id, 'za2805-sire.png', 'za2805-dam.png');
seedChildPedigree(za2818.id, za2818Sire.id, za2818Dam.id, 'za2818-sire.png', 'za2818-dam.png');
seedChildPedigree(za2911.id, za2911Sire.id, za2911Dam.id, 'za2911-sire.pdf', 'za2911-dam.pdf');
seedChildPedigree(the404.id, the404Sire.id, the404Dam.id, 'the404-sire.jpg', 'the404-dam.jpg');

console.log(`Seeded ${birds.length} birds, 4 child pedigrees (ZA 2805, ZA 2818, ZA 2911, The 404), and 1 standalone reference bird.`);
